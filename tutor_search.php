...
function searchOnlineTutors($subject) {
    global $conn;

    // Fetch tutors with 'online' tutoring mode and the provided subject
    $query = "SELECT * FROM tutor_info WHERE (Tutoring_Mode = 'Online' OR Tutoring_Mode = 'Online / In-Person') AND LOWER(MajorSubject) LIKE ?";
...
// Check if the subject query parameter is set and not empty
if (isset($_GET['q']) && !empty($_GET['q'])) {
    // Get the subject from the query parameter
    $search_query = $_GET['q'];
    $onlineTutors = searchOnlineTutors($search_query);
...
<body>
    ...
    <div id="dh-website">
        ...
        echo '<center><div class="col-12 edit"><h1 class="text-danger">No online tutors found for the subject: ' . $search_query . '</h1></div></center>';
    } else {
        // Display the online tutors for the provided subject
        foreach ($onlineTutors as $product) {
            // Display the tutor information here
      ...
      ?>
                           <!-- div start -->
                            ...
                            <!-- div end -->
                           <?php       // Add other details as needed
                        }
                    }
                } else {
                    // No subject provided, display a message
                    echo '<center><div class="col-12 edit"><h1 class="text-danger">Please provide a subject for the search</h1></div></center>';
                } ?>
                        </div>
                        <div class="t2526">
