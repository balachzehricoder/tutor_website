...
function searchOnlineTutors($subject, $location) {
    global $conn;

    // Fetch tutors with 'In-Person' tutoring mode, the provided subject, and location
    $query = "SELECT * FROM tutor_info WHERE (Tutoring_Mode = 'In-Person' OR Tutoring_Mode = 'Online / In-Person') AND LOWER(MajorSubject) LIKE ? AND LOWER(Location) LIKE ?";
    $stmt = $conn->prepare($query);
    $subject = '%' . strtolower($subject) . '%';
    $location = '%' . strtolower($location) . '%';
    $stmt->bind_param("ss", $subject, $location);
    $stmt->execute();
...
// Check if both subject and location parameters are set and not empty
if (isset($_GET['subject']) && !empty($_GET['subject']) && isset($_GET['location']) && !empty($_GET['location'])) {
    $search_subject = $_GET['subject'];
    $search_location = $_GET['location'];
    $onlineTutors = searchOnlineTutors($search_subject, $search_location);

    // Check if the onlineTutors array is empty after filtering
    if (empty($onlineTutors)) {
        echo '<center><div class="col-12 edit"><h1 class="text-danger">No in-person tutors found for the subject: ' . $search_subject . ' and location: ' . $search_location . '</h1></div></center>';
    } else {
        foreach ($onlineTutors as $tutor) {
...
<head>
    ...
    <!-- Title -->
    <title> Perth Tutors | Locations </title>
    <meta property="og:title" content="Perth Tutors | Locations" />
    <meta name="twitter:title" content="Perth Tutors | Locations" />
    ...
    <div id="dh-website">
        <main>
            <div class="t2506 dh-body">
                ...
                <section class="t2509">
                    <container class="t2510">
                        <div dh-transform="simple-animations" dh-transform-options="a:fadeInUp|b:0|c:0|d:600" class="t2530">
                            <h2 class="t2531">
                                
                            </h2>
                            <p class="t2534">Lorem Ipsum is simply dummy text of the printing and typesetting industry.</p>
                        </div>
                        <div class="t2511">
                       
                      
                           <?php   if (empty($onlineTutors)) {
        echo '<center><div class="col-12 edit"><h1 class="text-danger">No in-person tutors found for the subject: ' . $search_subject . ' and location: ' . $search_location . '</h1></div></center>';
    } else {
        foreach ($onlineTutors as $product) {
   
 ?>
                           <!-- div start -->
                            <div class="t2512">
                                <div class="t2515">
                                    <img src="<?php echo $product['uploadimg'] ?>" loading="lazy" alt="null" class="t2522">
                                    <div class="t2516">
                                    <span class="t719">$<?php echo $product['Tutoring_rate'] ?> / h</span>
                                        <span class="t717"><?php echo $product['Tutoring_Mode'] ?></span>
                                        <span class="t718"><?php echo $product['I_am_a'] ?></span>
                                        <span class="t720"><?php echo $product['MajorSubject'] ?></span>
                                        <span class="t721"><?php echo $product['Location'] ?></span>
                                    </div>
                                </div>
                                <div class="t2523">
                                    <h4 class="t2524"><?php echo $product['Name'] ?></h4>
                                    <p class="t2525"><?php echo $product['OtherSubject'] ?></p>
                                </div>
                                <div class="t2513">
                                    <a page-id="8" href="tutor-listing.php?id=<?php echo $id ?>" class="t2514 button-tertiary">Contact Tutor</a>
                                </div>
                            </div>
                            <!-- div end -->
                          <?php         
                        ...
                        <div class="t2526">
                            <a class="t2527 link-arrow-gold">
                                <span class="t2528">See More Tutors</span>
                                <img src="assets/72f6d43ce8f67fb507895e21994eecf4_269.svg" loading="lazy" alt="null" class="t2529">
                            </a>
                        </div>
                    </container>
                </section>
